    ...
    NesController controller2) : CpuEmulatorBus
{
    public override byte Peek(ushort addr)
    {
        if (addr == 0x4015)
            ...
            return mapper.PeekExpansion(addr);
        else if (addr >= 0x6000 && addr <= 0x7FFF)
            return mapper.PeekPrgRam(addr);
        else if (addr >= 0x8000)
            return mapper.PeekPrgRom(addr);
    ...
    public override byte Read(ushort addr)
    {
        if (addr == 0x4015)
        {
            ...
            return mapper.ReadExpansion(addr);
        else if (addr >= 0x6000 && addr <= 0x7FFF)
            return mapper.ReadPrgRam(addr);
        else if (addr >= 0x8000)
    ...
    public override void Write(ushort addr, byte value)
    {
        ...
        else if (addr >= 0x4000 && addr <= 0x4017 && addr != 0x4016)
        {
            ...
            mapper.WriteExpansion(addr, value);
        else if (addr >= 0x6000 && addr <= 0x7FFF)
            mapper.WritePrgRam(addr, value);
        else if (addr >= 0x8000)
